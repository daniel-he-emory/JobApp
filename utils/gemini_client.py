...
class GeminiClient:
    ...
    def __init__(self, api_key: str):
        ...
        self.logger.info(
            "Gemini client initialized with model: gemini-1.5-flash-latest")
    ...
    async def generate_content(self, prompt: str, is_json: bool = False) -> Union[str, Dict[str, Any]]:
        ...
        try:
            if not prompt or not prompt.strip():
                self.logger.warning(
                    "Empty prompt provided to generate_content")
            ...
            self.logger.debug(
                f"Generating content with prompt length: {len(prompt)} chars")
            ...
            self.logger.debug(
                f"Generated content length: {len(response_text)} chars")
    ...
    def count_tokens(self, text: str) -> int:
        ...
        try:
            ...
            self.logger.debug(
                f"Token count for text ({len(text)} chars): {token_count}")
            return token_count
            ...
            self.logger.debug(
                f"Using fallback token estimation: {estimated_tokens}")
    ...
    async def generate_with_retry(self, prompt: str, is_json: bool = False,
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                if attempt < max_retries:
                    ...
                    self.logger.warning(
                        f"Empty response on attempt {attempt + 1}, retrying in {wait_time}s")
        ...
        self.logger.error(
            f"Failed to generate content after {max_retries + 1} attempts. Last error: {last_error}")
        return {} if is_json else ""
...
def create_gemini_client_from_config(config: Dict[str, Any]) -> GeminiClient:
    ...
    try:
        ...
        logging.getLogger(__name__).info(
            "Gemini client created successfully from config")
        return client
    except Exception as e:
