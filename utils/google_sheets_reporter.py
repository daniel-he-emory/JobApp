...
class GoogleSheetsReporter:
    ...
    def _initialize_service(self):
        ...
        try:
            ...
            self.logger.error(
                f"Error initializing Google Sheets service: {str(e)}")
            self.service = None
    ...
    def _get_credentials(self) -> Optional[Credentials]:
        ...
        try:
            ...
            if os.path.exists(self.token_path):
                ...
                self.logger.debug(
                    "Loaded existing credentials from token.json")
            ...
            if not creds or not creds.valid:
                if creds and creds.expired and creds.refresh_token:
                    try:
                        ...
                        self.logger.info(
                            "Refreshed expired Google credentials")
                    except Exception as e:
                        self.logger.warning(
                ...
                if not creds:
                    if not os.path.exists(self.credentials_path):
                        ...
                        self.logger.error(
                            "Please download credentials from Google Cloud Console and save as google_credentials.json")
                        return None
                    ...
                    try:
                        ...
                        self.logger.info(
                            "Completed OAuth2 flow via out-of-band authorization")
    ...
    def _ensure_headers(self) -> bool:
        ...
        try:
            ...
            expected_headers = ['Date', 'Platform',
                                'Job Title', 'Company', 'Job URL', 'Status']
            ...
            if e.resp.status == 404:
                ...
                self.logger.error(
                    "Permission denied. Please share the spreadsheet with your service account")
            else:
    ...
    def append_applications(self, summary: dict) -> bool:
        ...
        if not self.service:
            self.logger.warning(
                "Google Sheets service not available, skipping reporting")
            return False
        ...
        try:
            ...
            self.logger.info(
                f"Successfully reported {updated_rows} job applications to Google Sheets")
            return True

        except HttpError as e:
            if e.resp.status == 429:
                self.logger.warning(
                    "Google Sheets API rate limit exceeded, retrying...")
            ...
            self.logger.error(
                f"Error appending applications to Google Sheets: {str(e)}")
    ...
    def test_connection(self) -> bool:
        ...
        try:
            ...
            self.logger.error(
                f"Google Sheets connection test failed: {str(e)}")
            return False
