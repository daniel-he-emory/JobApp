...
class GreenHouseEmailVerifier:
    ...
    def find_verification_email(self, timeout_minutes: int = 5) -> Optional[str]:
            ...
            try:
                ...
                for criteria in search_criteria:
                    ...
                    if status == 'OK' and messages[0]:
                        ...
                        for msg_id in reversed(message_ids[-10:]):  # Check last 10 emails
                            verification_link = self._extract_verification_link(mail, msg_id)
        ...
        self.logger.warning(f"No verification email found within {timeout_minutes} minutes")
        return None
    ...
    async def handle_greenhouse_verification(self, page, timeout_minutes: int = 5) -> bool:
        ...
        try:
            ...
            self.logger.info("Detected Greenhouse verification page, checking email...")
            
            # Find verification email
            verification_link = self.find_verification_email(timeout_minutes)
            
            if verification_link:
                self.logger.info(f"Navigating to verification link: {verification_link}")
            ...
            self.logger.error(f"Error handling Greenhouse verification: {str(e)}")
            return False