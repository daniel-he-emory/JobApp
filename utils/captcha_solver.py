...
class CaptchaSolver:
    ...
    async def solve_recaptcha_v2(self, site_key: str, page_url: str) -> str:
        ...
        if self.service_name == "2captcha":
            ...
            raise CaptchaError(f"reCAPTCHA v2 not supported for {self.service_name}")
    ...
    async def _solve_2captcha_recaptcha_v2(self, site_key: str, page_url: str) -> str:
        ...
        Returns:
            The solved CAPTCHA token string
        """
        self.logger.info(f"Starting 2Captcha reCAPTCHA v2 solve for {page_url}")
            ...
            try:
                async with session.post(self.endpoints["2captcha"]["submit"], data=submit_data) as response:
                    if response.status != 200:
                        raise CaptchaError(f"Failed to submit CAPTCHA: HTTP {response.status}")
                    
                    result = await response.json()
                    
                    if result.get("status") != 1:
                        error_msg = result.get("error_text", "Unknown error")
                        raise CaptchaError(f"2Captcha submission failed: {error_msg}")
                    
                    captcha_id = result.get("request")
                    if not captcha_id:
                        raise CaptchaError("No CAPTCHA ID returned from 2Captcha")
                    
                    self.logger.info(f"CAPTCHA submitted successfully. ID: {captcha_id}")
                    
            except aiohttp.ClientError as e:
                raise CaptchaError(f"Network error during CAPTCHA submission: {str(e)}")
            except Exception as e:
                raise CaptchaError(f"Unexpected error during CAPTCHA submission: {str(e)}")
        ...
        while attempt < max_attempts:
            ...
            try:
                async with session.get(self.endpoints["2captcha"]["result"], params=result_data) as response:
                    if response.status != 200:
                        self.logger.warning(f"Poll attempt {attempt}: HTTP {response.status}")
                        continue
                    ...
                    if result.get("status") == 1:
                        ...
                        if token:
                            self.logger.info(f"CAPTCHA solved successfully after {attempt} attempts")
                            return token
                        else:
                            raise CaptchaError("CAPTCHA marked as solved but no token returned")
                    
                    elif result.get("request") == "CAPCHA_NOT_READY":
                        # Still processing, continue polling
                        self.logger.debug(f"Poll attempt {attempt}: CAPTCHA not ready yet")
                        continue
                    
                    else:
                        # Error occurred
                        error_msg = result.get("error_text", result.get("request", "Unknown error"))
                        raise CaptchaError(f"2Captcha solving failed: {error_msg}")
                        
            except aiohttp.ClientError as e:
                self.logger.warning(f"Poll attempt {attempt}: Network error - {str(e)}")
                continue
            except Exception as e:
                if "CAPTCHA" in str(e):
                    raise  # Re-raise CAPTCHA-specific errors
                self.logger.warning(f"Poll attempt {attempt}: Unexpected error - {str(e)}")
                continue
        
        raise CaptchaError(f"CAPTCHA solving timeout after {max_attempts} attempts (5 minutes)")
    ...
    async def _solve_anticaptcha_recaptcha_v2(self, site_key: str, page_url: str) -> str:
        ...
        Returns:
            The solved CAPTCHA token string
        """
        self.logger.info(f"Starting Anti-Captcha reCAPTCHA v2 solve for {page_url}")
            ...
            try:
                async with session.post(self.endpoints["anticaptcha"]["submit"], json=submit_data) as response:
                    ...
                    if result.get("errorId") != 0:
                        ...
                        raise CaptchaError(f"Anti-Captcha submission failed: {error_msg}")
                    
                    task_id = result.get("taskId")
                    if not task_id:
                        raise CaptchaError("No task ID returned from Anti-Captcha")
                    
                    self.logger.info(f"CAPTCHA submitted successfully. Task ID: {task_id}")
        ...
        while attempt < max_attempts:
            ...
            try:
                async with session.post(self.endpoints["anticaptcha"]["result"], json=result_data) as response:
                    ...
                    if result.get("errorId") != 0:
                        error_msg = result.get("errorDescription", "Unknown error")
                        raise CaptchaError(f"Anti-Captcha polling failed: {error_msg}")
                    ...
                    if status == "ready":
                        ...
                        if token:
                            ...
                            raise CaptchaError("CAPTCHA marked as ready but no token returned")
                    
                    elif status == "processing":
                        # Still processing, continue polling
                        self.logger.debug(f"Poll attempt {attempt}: CAPTCHA still processing")
                        continue
                    
                    else:
                        raise CaptchaError(f"Unexpected status from Anti-Captcha: {status}")
    ...
    async def get_balance(self) -> float:
        ...
        if self.service_name == "2captcha":
            ...
            raise CaptchaError(f"Balance check not supported for {self.service_name}")
        ...
        async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=30)) as session:
            ...
            try:
                async with session.get(self.endpoints["2captcha"]["result"], params=params) as response:
                    if response.status != 200:
                        raise CaptchaError(f"Failed to get balance: HTTP {response.status}")
                    ...
                    if result.get("status") == 1:
                        ...
                        error_msg = result.get("error_text", "Unknown error")
                        raise CaptchaError(f"Balance check failed: {error_msg}")
                        
            except aiohttp.ClientError as e:
                raise CaptchaError(f"Network error during balance check: {str(e)}")
...
def create_captcha_solver_from_config(config: Dict[str, Any]) -> Optional[CaptchaSolver]:
    ...
    if not api_key or api_key == 'your_2captcha_api_key_here':
        logging.getLogger(__name__).warning("CAPTCHA solving enabled but no valid API key configured")
        return None
    
    try:
        return CaptchaSolver(service_name, api_key)
    except Exception as e:
        logging.getLogger(__name__).error(f"Failed to create CAPTCHA solver: {str(e)}")
        return None