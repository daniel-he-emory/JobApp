...
class ProxyConfig:
    ...
    # Provider type: 'smartproxy', 'brightdata', 'oxylabs', etc.
...
class ResidentialProxyManager:
    ...
    def __init__(self, provider: str, username: str, password: str,
        ...
        if self.provider not in self.PROVIDER_CONFIGS:
            raise ValueError(
                f"Unsupported provider: {provider}. Supported: {list(self.PROVIDER_CONFIGS.keys())}")

        self.provider_config = self.PROVIDER_CONFIGS[self.provider]
        self.logger.info(
            f"Initialized residential proxy manager for {provider}")
...
class ProxyManager:
    ...
    def rotate_proxy(self) -> Optional[ProxyConfig]:
        ...
        while attempts < max_attempts:
            self.current_proxy_index = (
                self.current_proxy_index + 1) % len(self.proxy_configs)
            ...
            if proxy_key not in self.failed_proxies:
                ...
                self.logger.info(
                    f"Rotated to proxy: {proxy.host}:{proxy.port}")
                return proxy
        ...
        if attempts >= max_attempts:
            self.logger.warning(
                "All proxies have failed, resetting failed list")
    ...
    def validate_proxy(self, proxy: ProxyConfig, timeout: int = 10) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.logger.info(
                    f"Proxy {proxy.host}:{proxy.port} working, IP: {ip_data.get('origin')}")
                return True
            else:
                self.logger.warning(
                    f"Proxy {proxy.host}:{proxy.port} returned status: {response.status_code}")
                return False

        except Exception as e:
            self.logger.error(
                f"Proxy validation failed for {proxy.host}:{proxy.port}: {str(e)}")
            return False
    ...
    def validate_all_proxies(self) -> List[ProxyConfig]:
        ...
        self.logger.info(
            f"Validated proxies: {len(working_proxies)}/{len(self.proxy_configs)} working")
    ...
    if smartproxy_config.get('enabled', False):
        try:
            ...
            residential_manager = ResidentialProxyManager(
                ...
                sticky_session=smartproxy_config.get(
                    'session_type') == 'sticky'
            )
            ...
            logging.getLogger(__name__).info(
                "Created residential proxy manager for smartproxy")
            return manager

        except Exception as e:
            logging.getLogger(__name__).error(
                f"Failed to create smartproxy proxy manager: {e}")
