...
class JobAgent(ABC):
    ...
    async def initialize_browser(self, headless: bool = None) -> None:
        ...
        if proxy_config.get('enabled', False):
            try:
                ...
                self.proxy_manager = create_proxy_manager_from_config(self.config)
                ...
                if self.proxy_manager:
                    ...
                    if current_proxy:
                        ...
                        playwright_proxy_config = self.proxy_manager.get_playwright_proxy_config(current_proxy)
                        launch_options['proxy'] = playwright_proxy_config
                        self.logger.info(f"Using proxy: {current_proxy.host}:{current_proxy.port}")
                    else:
                        self.logger.warning("No working proxy available")
                else:
                    self.logger.info("Proxy enabled but no valid configuration found")
                    
            except Exception as e:
                self.logger.warning(f"Could not configure proxy: {e}, proceeding without proxy")
        else:
        ...
        if captcha_config.get('enabled', False):
            try:
                ...
                self.captcha_solver = create_captcha_solver_from_config(self.config)
                if self.captcha_solver:
                    self.logger.info("CAPTCHA solver initialized and ready")
                else:
                    self.logger.warning("CAPTCHA solver enabled but could not initialize")
            except Exception as e:
                self.logger.warning(f"Could not initialize CAPTCHA solver: {e}")
    ...
    async def handle_captcha_if_present(self) -> bool:
        ...
        try:
            ...
            if recaptcha_iframe:
                self.logger.info("reCAPTCHA v2 detected, attempting to solve...")
            ...
            if recaptcha_v3:
                self.logger.info("reCAPTCHA v3 detected, attempting to solve...")
            ...
            if hcaptcha:
                self.logger.warning("hCaptcha detected but not currently supported")
                return False
    ...
    async def _solve_recaptcha_v2(self) -> bool:
        ...
        try:
            ...
            if not site_key:
                self.logger.error("Could not extract reCAPTCHA site key from page")
                return False
            
            self.logger.info(f"Extracted reCAPTCHA site key: {site_key[:20]}...")
            ...
            if not token:
                self.logger.error("CAPTCHA solving service returned empty token")
                return False
    ...
    async def _extract_recaptcha_site_key(self) -> Optional[str]:
        ...
        try:
            ...
            matches = re.findall(r'sitekey["\']?\s*[:=]\s*["\']([^"\']+)["\']', content)
            if matches:
                return matches[0]
