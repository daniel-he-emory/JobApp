...
class TestStateManagerSQLite:
    ...
    def test_init_sqlite(self, temp_dir):
        ...
        state_manager = StateManager(storage_type="sqlite", file_path=str(db_path))
    ...
    def test_init_sqlite_memory(self):
        ...
        state_manager = StateManager(storage_type="sqlite", file_path=":memory:")
    ...
    def test_get_recent_applications(self, temp_dir):
        ...
        # Record applications
        state_manager.record_application("job1", "linkedin", title="Engineer 1")
        state_manager.record_application("job2", "wellfound", title="Engineer 2")
        state_manager.record_application("job3", "linkedin", title="Engineer 3")
    ...
    def test_thread_safety(self, temp_dir):
        ...
        def worker(job_id):
            result = state_manager.record_application(f"job{job_id}", "linkedin")
...
class TestStateManagerCSV:
    ...
    def test_init_csv(self, temp_dir):
        ...
        state_manager = StateManager(storage_type="csv", file_path=str(csv_path))
...
class TestStateManagerGeneral:
    ...
    def test_invalid_storage_type(self, temp_dir):
        ...
        with pytest.raises(ValueError, match="storage_type must be 'sqlite' or 'csv'"):
            StateManager(storage_type="invalid", file_path=str(temp_dir / "test"))
    
    def test_directory_creation(self, temp_dir):
        """Test that parent directories are created automatically"""
        nested_path = temp_dir / "nested" / "directory" / "test.db"
        state_manager = StateManager(storage_type="sqlite", file_path=str(nested_path))
...
class TestStateManagerConcurrency:
    ...
    def test_concurrent_duplicate_detection(self, temp_dir):
        ...
        def try_record_same_job():
            result = state_manager.record_application("duplicate_job", "linkedin")
            results.append(result)
    ...
    def test_concurrent_stats_access(self, temp_dir):
        ...
        def write_applications():
            for i in range(100, 110):
                result = state_manager.record_application(f"job{i}", "wellfound")
        ...
        assert all(isinstance(count, int) and count >= 10 for count in stats_results)
