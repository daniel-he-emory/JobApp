...
class TestJobApplicationOrchestrator:
    ...
    def test_orchestrator_initialization(self, config_file):
        ...
        orchestrator = JobApplicationOrchestrator(str(config_file), dry_run=False)
    ...
    def test_orchestrator_dry_run_mode(self, config_file):
        ...
        orchestrator = JobApplicationOrchestrator(str(config_file), dry_run=True)
    ...
    async def test_run_automation_with_mock_agents(self, config_file):
        ...
        assert all(r['platform'] == 'MockAgent' for r in result['platform_results'])
...
class TestConfigIntegration:
    ...
    def test_config_with_environment_override(self, temp_dir):
        ...
        with patch.dict('os.environ', {'LINKEDIN_EMAIL': 'env@example.com'}):
            orchestrator = JobApplicationOrchestrator(str(config_path))
            linkedin_creds = orchestrator.config_loader.get_credentials('linkedin')
    ...
    async def test_duplicate_application_prevention(self, config_file):
        ...
        class DuplicateJobAgent:
            ...
            async def search_jobs(self, criteria):
                ...
                job = JobPosting("duplicate_job", "Engineer", "TechCorp", "Remote", "url1")
                return [job, job]
        ...
        assert orchestrator.state_manager.has_applied("duplicate_job", "test_platform")
...
class TestErrorHandlingAndRecovery:
    ...
    async def test_partial_platform_failure(self, config_file):
        ...
        orchestrator.get_enabled_platforms = Mock(return_value=['success_platform', 'failing_platform'])
        ...
        success_results = [r for r in result['platform_results'] if r['errors'] == 0]
        error_results = [r for r in result['platform_results'] if r['errors'] > 0]
    ...
    async def test_state_manager_recovery(self, temp_dir):
        ...
        assert orchestrator.state_manager is not None
        assert not orchestrator.state_manager.has_applied("test_job", "linkedin")
        
        # Record and verify application
        orchestrator.state_manager.record_application("test_job", "linkedin", "Engineer", "TechCorp")
