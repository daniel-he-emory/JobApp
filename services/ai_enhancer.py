...
class AIEnhancer:
    ...
    def __init__(self, gemini_client: GeminiClient, structured_resume: Dict[str, Any], 
        ...
        if not self.prompts:
            raise AIEnhancementError("No prompt templates found in configuration")
    ...
    async def score_job_relevance(self, job_posting: JobPosting) -> Dict[str, Any]:
        ...
        try:
            ...
            if not prompt_template:
                raise AIEnhancementError("score_job_relevance prompt template not found in config")
            ...
            if not isinstance(result, dict):
                raise AIEnhancementError("AI returned invalid response format for job scoring")
            
            # Validate required fields
            if 'score' not in result or 'reasoning' not in result:
                raise AIEnhancementError("AI response missing required fields: score, reasoning")
            ...
            try:
                ...
                raise AIEnhancementError(f"Invalid score value: {result.get('score')}")
            ...
            if isinstance(e, AIEnhancementError):
                ...
                raise AIEnhancementError(f"Error scoring job relevance: {str(e)}")
    ...
    async def generate_cover_letter(self, job_posting: JobPosting) -> str:
        ...
        try:
            ...
            if not prompt_template:
                raise AIEnhancementError("generate_cover_letter prompt template not found in config")
            ...
            if not isinstance(cover_letter, str) or not cover_letter.strip():
                raise AIEnhancementError("AI returned empty or invalid cover letter")
            ...
            if isinstance(e, AIEnhancementError):
                ...
                raise AIEnhancementError(f"Error generating cover letter: {str(e)}")
    ...
    async def optimize_resume_section(self, job_posting: JobPosting, 
        ...
        try:
            ...
            if not prompt_template:
                raise AIEnhancementError("optimize_resume_keywords prompt template not found in config")
            ...
            # Replace placeholders in prompt
            prompt = prompt_template.replace('[JOB_DESCRIPTION]', job_description)
            prompt = prompt.replace('[RESUME_SECTION]', resume_section_text.strip())
            ...
            # Validate response structure
            if not isinstance(result, dict):
                raise AIEnhancementError("AI returned invalid response format for resume optimization")
            
            # Extract optimized text
            optimized_text = result.get('optimized_text')
            if not optimized_text or not isinstance(optimized_text, str):
                raise AIEnhancementError("AI response missing or invalid 'optimized_text' field")
            ...
            if isinstance(e, AIEnhancementError):
                ...
                raise AIEnhancementError(f"Error optimizing resume section: {str(e)}")
    ...
    def _extract_job_description(self, job_posting: JobPosting) -> str:
        ...
        if job_posting.description:
            ...
            description_parts.append(f"Job Description: {job_posting.title} position at {job_posting.company}")
    ...
    def _format_resume_for_prompt(self) -> str:
        ...
        try:
            ...
            if self.structured_resume.get('full_name'):
                profile_parts.append(f"Name: {self.structured_resume['full_name']}")
            ...
            if self.structured_resume.get('summary'):
                profile_parts.append(f"Professional Summary:\n{self.structured_resume['summary']}")
